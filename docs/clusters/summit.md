...
1. To run a 32-core job for 36 hours on a single summit-ics node:
```bash
#SBATCH --qos=summit-ics
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=32
#SBATCH --time=36:00:00
```

2. To run a 56-core job across two summit-sha nodes for seven days:
```bash
#SBATCH --qos=summit-sha
#SBATCH --nodes=2
...
3. To run a 16-core job for 36 hours on a single summit-curc-gpu node, using all three gpus:
```bash
#SBATCH --qos=summit-curc-gpu
#SBATCH --nodes=1
...
7. To start a 2-hr interactive job on one core on a summit-ceae node, run this at the command line:
```bash
sinteractive --qos=summit-ceae --export=NONE --time=02:00:00
```
...
Run a 6-hour preemptable python job on 32 cores without specifying a partition (job will run on any available compute partitions on Summit, regardless of features, so long as they have at least 16 cores each).
...
Same as Example 1, but specify a specific partition (‘summit-ccn’) (job will only run on summit-ccn nodes)

```bash
#!/bin/bash
#SBATCH --time=06:00:00
#SBATCH --qos=preemptable
#SBATCH --partition=summit-ccn
#SBATCH --job-name=test
#SBATCH --nodes=2
#SBATCH --ntasks=32
#SBATCH --output=test.%j.out
...
The ‘summit’ QoS: Note that as of 1 March, 2018, the “preemptable” qos replaces the previous low-priority QoS, “summit”, which is no longer active.
