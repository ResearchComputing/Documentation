...
The `gpu-testing` QoS provides access to limited GPU resources for the purpose of verifying GPU workflows and building GPU-accelerated applications. Please note that `gpu-testing` QoS must be used in conjunction with a chosen GPU partition and GPU type. For a list of resources that are available via `gpu-testing` as well as limitations of the QoS, see the sections [Quality of Service (qos)](#quality-of-service-qos) and [General Resources (gres)](#general-resources-gres). 
