# Trainings and Consults

CU Research Computing (CURC) offers a variety of educational resources and support to faculty, students, and researchers. These include live trainings (in-person, virtual, & hybrid), online trainings, and regular office hours (in-person & virtual).

## Trainings

```{toctree}
:maxdepth: 1
current-sem-trainings
online-trainings

```

## Weekly Office Hours

Join the CURC Weekly Office Hours during the academic year and get one-on-one support with a member of the User Support Team. Whether you're trying to optimize your workflows on Alpine and Blanca or just getting started with your first High-Performance Computing project, we’re here to help you bridge the gap between "it’s broken" and "it’s running."

Don't let technical bottlenecks stall your progress. Come with your questions, leave with a plan, and get your research back on track.

```{button-link} https://www.colorado.edu/crdds/events-consult
:align: center
:color: secondary
:outline:
Register for Office Hours {octicon}`calendar`
```

```{note}
If you are unable to meet during the weekly office hours, or if you need a consultation outside of the academic year, please use the [support request form](https://colorado.service-now.com/req_portal?id=ucb_sc_rc_form) to request a virtual consultation. 

```


