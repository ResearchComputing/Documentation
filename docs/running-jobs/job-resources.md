...
| Total Memory       | The total memory required to complete the Job. <br> The total memory (per node requested) required to complete the job. The number of cores allocated to your job does not scale with the total memory requested, but you will be charged # cores = total memory / RAM per core. Units can be specified with the suffixes: K,M,G,T (default M) | --mem=memory |
