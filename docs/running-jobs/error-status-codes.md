...
| 19 | The requested runtime of the `<qos>` QoS must be greater than `<N>` hour(s). | You have not requested enough time to meet the minimum required for this QoS. | Increase the time requested for the job, or switch to a different QoS that supports shorter jobs. You can find a list of valid QoS values in our [Alpine QoS Documentation](../clusters/alpine/alpine-hardware.md#quality-of-service-qos). |
