...
3. Once your VS Code-Server session is ready, you can click “Connect to VS Code”. An interactive Jupyter session will be started in a new window.
![](OnDemand/vs_code_custom_launch)
...
When attempting to install an extension we suggest the following method (listed in order of preference):
...
- Install the application by dragging the downloaded file into the application (for visual representation, please see above video).
